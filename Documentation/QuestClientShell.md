...
`Assets/Plugins/Android/com/metalxr/questclient/MetalXRSurfaceDecoder.java` is the first Android-side skeleton for the future Surface decode path. It can create an external OES texture, configure MediaCodec with a `SurfaceTexture` target, queue H.264 frames, and release decoded output to the Surface. The Unity client can instantiate this backend only when explicitly launched with `--metalxr-surface-decode` or the Android intent extra `metalxr_surface_decode=true`; `Scripts/run-metalxr-playmode-workflow.sh` passes that extra when `METALXR_QUEST_SURFACE_DECODE=1` is set. The Quest app now gates this path to OpenGLES graphics devices because the Java side needs a current GLES context for external OES texture creation. With the default Vulkan-first Quest build, the opt-in logs the unsupported graphics backend and falls back to the active Image-plane decode path.

For GLES experiments, build a dedicated APK with:
