...
- The runtime now has an experimental IOSurface sidecar export path behind `METALXR_SWAPCHAIN_RESOURCE_MODE=iosurface`, `METALXR_FRAME_EXPORT_MODE=iosurface`, and `METALXR_ENABLE_EXPERIMENTAL_IOSURFACE_EXPORT=1`. Unity still receives its normal runtime-owned Metal swapchain texture; at frame submission time the runtime can lazily create per-image/per-eye IOSurface-backed 2D textures, GPU-blit the submitted array slice into them, and emit `IOSurfaceBGRA8` or `IOSurfaceRGBA8` records with `ioSurfaceId`, `frameSlotId`, `frameSlotGeneration`, and `frameSlotFence` instead of a raw payload file. When `METALXR_FRAME_EXPORT_SOCKET` is set, the IOSurface path can publish records without `METALXR_FRAME_EXPORT_DIR`; file records remain optional debug artifacts.
- `METALXR_FRAME_EXPORT_ACK_SOCKET` enables an explicit host release-ack channel for IOSurface slots. The runtime marks a slot pending after the Metal command buffer completes and waits for a matching `frameSlotGeneration` release ack before copying a later frame into the same IOSurface. `METALXR_FRAME_SLOT_ACK_TIMEOUT_MS` controls the wait budget and defaults to 50 ms; on timeout the runtime skips that export instead of overwriting a slot the host may still be encoding.
- Synchronization is still experimental, but the IOSurface path now has explicit runtime-host slot ownership in addition to the Metal command-buffer completion fence. If the application does not provide a Metal command queue in `XrGraphicsBindingMetalKHR`, the runtime creates an internal command queue for debug readback and IOSurface sidecar validation. When an application does provide a command queue, the runtime validates that a Metal device can be obtained from it and returns `XR_ERROR_GRAPHICS_DEVICE_INVALID` for invalid queues.
...
The probe also sets `METALXR_FRAME_DUMP_DIR` and, by default, `METALXR_FRAME_EXPORT_DIR`. It fails unless `frame_000001.txt` is written with projection frame metadata and `frames.jsonl` plus non-empty left/right fixture payloads are written by the frame export path. For IOSurface validation, the probe can set `METALXR_PROBE_FRAME_EXPORT_DIR=` and `METALXR_PROBE_FRAME_EXPORT_SOCKET=/tmp/...sock` to verify socket-only `ioSurfaceId` records without debug frame export files.
